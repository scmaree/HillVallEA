...
iAMaLGaM-Univariate as part of HillVallEA
...
hillvallea::iamalgam_univariate_t::~iamalgam_univariate_t(){};

// Algorithm Name
std::string hillvallea::iamalgam_univariate_t::name() const { return "i-AMaLGaM-univariate"; }


hillvallea::optimizer_pt hillvallea::iamalgam_univariate_t::clone() const
{

  iamalgam_univariate_pt opt = std::make_shared<iamalgam_univariate_t>(number_of_parameters, lower_param_bounds, upper_param_bounds, init_univariate_bandwidth, fitness_function, rng);
...
void hillvallea::iamalgam_univariate_t::initialize_from_population(population_pt pop)
{
  this->pop = pop;
  multiplier = 1.0;
...
// returns the popsize, mininum is such that the selection contains at least two solutions.
size_t hillvallea::iamalgam_univariate_t::recommended_popsize(const size_t problem_dimension) const
{
  return (size_t) std::max((double) ((size_t) ((2.0 / selection_fraction) + 1)), 4.0*pow((double)problem_dimension, 0.5));
}

void hillvallea::iamalgam_univariate_t::update_distribution_multiplier(double & multiplier, const bool improvement, double & no_improvement_stretch, const double & sample_success_ratio, const double sdr) const
{
...
bool hillvallea::iamalgam_univariate_t::checkTerminationCondition()
{
  ...
  // stall size is a double for simplicity.  upper: recc popsize (
  // stallsize = (size_t)(10 + floor(30.0 * number_of_parameters / recommended_popsize(number_of_parameters)));  // Stall time(for termination criterion)
  /* double TolHistFun = 1e-5;
  double bestf_NEmin = 1e308;
  double bestf_NEmax = -1e308;
  for (size_t i = (size_t) std::max((double) (this->bestf_NE.size() - stallsize), (double) 0); i < this->bestf_NE.size(); ++i)
  {
  ...
  if (terminate_for_param_std_mean_zero || terminate_on_parameter_std || terminate_on_fitness_std || terminate_on_distribution_multiplier || terminate_on_slow_improvement)
  {
     active = false;
     return !active;
  }
...
void hillvallea::iamalgam_univariate_t::estimate_sample_parameters()
{
 ...
 // Compute sample mean and sample covariance
  ...
  {
    pop->covariance_univariate(mean, generational_covariance);
  }
  ...
  else
  {
    ...
    for (size_t i = 0; i < number_of_parameters; i++) {
      aggregated_covariance[i][i] = (1.0 - eta_s)*aggregated_covariance[i][i] + eta_s*generational_covariance[i][i];
    }
  }
...
size_t hillvallea::iamalgam_univariate_t::sample_new_population(const size_t sample_size)
{
...
// Compute the SDR
//--------------------------------------------------------------------------------------
double hillvallea::iamalgam_univariate_t::getSDR(const solution_t & best, const vec_t & mean, matrix_t & inverse_chol) const
{
  size_t i;
...
//------------------------------------------------------------------------
void hillvallea::iamalgam_univariate_t::apply_ams_to_population(const size_t number_of_ams_solutions, const double ams_factor, const vec_t & ams_direction)
{
